...
public class Main2 {
    ...
    public static long[][] fazer_arvore(long[] array){
        long[][] tree = new long[log2(array.length) + 1][array.length];
        int marcador = 0;

        for (int i = 0; i < log2(array.length) + 1; i++){
            for (int j = 0; j < Math.pow(2,(log2(array.length)-i)); j++){
                if(i == 0){
                    tree[i][j] = hashcode(array[j]);
                } else {
                    tree[i][j] = hashcode(tree[i-1][marcador], tree[i-1][marcador + 1]);
                    marcador += 2;
                }
            }
            marcador = 0;
    ...
    public static void imprimir(long[][] tree){
        for (int i = tree.length - 1; i >= 0; i--){
            for (int j = 0; j < Math.pow(2, tree.length - i - 1); j++){
                System.out.printf("%d\n", tree[i][j]);
            }
        }
    }
